...
5. **Write design doc** — save to `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` (do not commit it unless the user asks)
...
- Leave the design document uncommitted — do not commit docs unless the user explicitly asks
...
When a question is genuinely visual, use the harness's native affordances instead of describing visuals in prose: `AskUserQuestion` option `preview`s for side-by-side mockups and layout comparisons, or the Artifact tool for richer HTML mockups and diagrams.

The test: **would the user understand this better by seeing it than reading it?** A question *about* a UI topic is not automatically a visual question. "What does personality mean in this context?" is conceptual — plain text. "Which wizard layout works better?" is visual — show it.
