...
| `requiresUserSpecification` | boolean | no | Set by `writing-plans` when the user's brief says WHAT should be verified but not HOW. Signals to `checking-gates` that it must hand off to `specifying-gates` before running verification. Removed automatically once `specifying-gates` has captured the HOW. |
| `gateScope` | "once" \| "per-target" \| "one-then-all" \| string | no | **Advisory** hint for how many times / across how many targets the gate should run — set by `specifying-gates` (Q3). Not read by the executor: multi-target and "one-then-all" behavior is realized by creating separate gate tasks chained with `blockedBy`, not by `checking-gates` iterating this field. Free-form string allowed. |
| `failurePolicy` | "stop-plan" \| "reopen-continue" \| "log-continue" | no | What happens if the gate fails. Set by `specifying-gates` (Q4). |
| `subagentBrief` | string | no | Exact prompt/briefing for the dispatch subagent when the gate's proof mechanism is "subagent". Set by `specifying-gates` (Q5). Agent MUST pass this verbatim at dispatch — substituting a shorter version defeats the purpose. |
